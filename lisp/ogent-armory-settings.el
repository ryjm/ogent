...
(defun ogent-armory-settings--plist-merge (base change)
...
(defun ogent-armory-settings-update (directory &rest change)
  "Apply CHANGE to Armory settings for DIRECTORY."
  (ogent-armory-settings-write directory change :merge t))
...
(defvar ogent-armory-settings-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c e") #'ogent-armory-settings-edit)
    (define-key map "E" #'ogent-armory-settings-export-current)
    (define-key map (kbd "C-c E") #'ogent-armory-settings-export-current)
    (define-key map "I" #'ogent-armory-settings-import-current)
    (define-key map (kbd "C-c I") #'ogent-armory-settings-import-current)
    (define-key map "o" #'ogent-armory-onboard-current)
    (define-key map (kbd "C-c o") #'ogent-armory-onboard-current)
    (define-key map "b" #'ogent-armory-backup-current)
    (define-key map (kbd "C-c b") #'ogent-armory-backup-current)
    (define-key map "?" #'ogent-armory-help)
    (define-key map (kbd "C-c ?") #'ogent-armory-help)
    (define-key map "g" #'ogent-armory-settings-refresh)
    (define-key map (kbd "C-c g") #'ogent-armory-settings-refresh)
  ...
  (let* ((settings (ogent-armory-settings-read ogent-armory-settings--root))
         ...
         (value (cond
                 ((eq key :default-provider)
                  (ogent-armory-settings--read-provider
                   "Default provider: "
                   (format "%s" (or current ""))))
                 ((eq key :default-model)
                  (ogent-armory-settings--read-model
...
(defun ogent-armory-settings--read-provider (&optional prompt current)
  "Read a Armory provider using PROMPT and CURRENT."
  (completing-read (or prompt "Provider: ")
                   (ogent-armory-settings--adapter-ids)
                   nil nil nil nil current))

(defun ogent-armory-settings--read-model
...
fallback default."
  (let* ((adapter (ogent-armory-adapter-resolve-provider provider))
...
Use NAME as the Armory name.
Use DEFAULT-PROVIDER, DEFAULT-MODEL, DEFAULT-EFFORT, and RUNTIME
to seed settings.
Use TEAM, or nil for `ogent-armory-onboard-default-team', as the agent roster."
  (interactive
   (let* ((directory (read-directory-name "Armory directory: "))
          ...
          (model (ogent-armory-settings--read-model
                  provider nil nil t))
...
(defvar ogent-armory-help-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c d") #'ogent-armory-demo)
    (define-key map "s" #'ogent-armory-settings)
    (define-key map (kbd "C-c s") #'ogent-armory-settings)
    (define-key map "q" #'quit-window)
    map)
  ...
  (let ((buffer (get-buffer-create ogent-armory-help-buffer-name))
    ...
    (with-current-buffer buffer
      (let ((inhibit-read-only t))
        ...
        (insert "Armory display buffers expose local actions through C-c chords, leaving Evil normal-state movement keys available.\n\n")
