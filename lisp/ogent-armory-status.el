...
(defun ogent-armory-status-help ()
  ...
  (with-help-window "*Ogent Armory Status Help*"
    ...
    (princ "Evil normal and motion states expose the same Armory keys; j/k still move by line and ZZ/ZQ quit.\n")))
...
(defconst ogent-armory-status--evil-bindings
  '(("g" ogent-armory-status-refresh)
    ...
    ("G" ogent-armory-status-open-gastown)
    ("R" ogent-armory-status-run)
    ("m" ogent-armory-status-dispatch)
    ...
    ("ZZ" quit-window)
    ("ZQ" quit-window))
  "Evil normal and motion state bindings for Armory status buffers.")
...
(defun ogent-armory-status--evil-local-keys ()
  ...
  (dolist (binding ogent-armory-status--evil-bindings)
    (ogent-armory-status--evil-bind-local (car binding) (cadr binding))))
