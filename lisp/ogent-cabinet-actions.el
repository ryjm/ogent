...
(defvar ogent-cabinet-actions-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c a") #'ogent-cabinet-actions-approve-at-point)
    (define-key map "r" #'ogent-cabinet-actions-reject-at-point)
    (define-key map (kbd "C-c r") #'ogent-cabinet-actions-reject-at-point)
    (define-key map "A" #'ogent-cabinet-actions-approve-buffer)
    (define-key map (kbd "C-c A") #'ogent-cabinet-actions-approve-buffer)
    (define-key map "R" #'ogent-cabinet-actions-reject-buffer)
    (define-key map (kbd "C-c R") #'ogent-cabinet-actions-reject-buffer)
    (define-key map "d" #'ogent-cabinet-actions-dispatch-buffer)
    (define-key map (kbd "C-c d") #'ogent-cabinet-actions-dispatch-buffer)
    (define-key map "g" #'ogent-cabinet-actions-refresh)
    (define-key map (kbd "C-c g") #'ogent-cabinet-actions-refresh)
...
(defun ogent-cabinet-actions--schedule-task (root conversation-id action triggering-agent)
  ...
  (let* ((id (format "%s-%s"
         ...
         (schedule-key (ogent-cabinet-schedule-key
                        target
                        'task
                        (plist-get action :id)
                        (ogent-cabinet-schedule-parse-time datetime)))
