...
| `C-c A` | Apps |
| `C-c G` | Graph/status |
| `C-c e` | Edit Cabinet metadata |
| `C-c n` / `C-c p` | Move between actionable rows |

In Evil normal state, bare Vim navigation/search keys keep their Evil meanings. Cabinet display actions use the `C-c` chords above, while `RET`, `TAB`, `M-n`/`M-p`, `q`, `ZZ`, and `ZQ` remain available where documented.
