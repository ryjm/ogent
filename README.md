...
ogent includes first-class support for **Gas Town**, a multi-agent workspace manager for coordinating AI agents across multiple projects. Gas Town provides:

- **Mayor**: Global coordinator that dispatches work across rigs
- **Witness**: Per-rig worker lifecycle manager
- **Refinery**: Merge queue processor for integrating agent work
- **Polecats**: Worker agents with dedicated git worktrees
- **Beads**: Issue tracking system shared across all agents

### Quick Start

Enable Gas Town mode globally:
...
| Command | Description |
|---------|-------------|
| `M-x ogent-gastown-dispatch` | Open the Gas Town transient menu |
| `M-x ogent-gastown-status` | Show full status dashboard |
| `M-x ogent-gastown-show-hook` | View currently hooked work |
| `M-x ogent-gastown-show-mail` | View agent mail inbox |
| `M-x ogent-gastown-send-mail` | Compose and send agent mail |
| `M-x ogent-gastown-show-convoy` | View convoy (batch work) status |
| `M-x ogent-gastown-show-ready` | Show issues ready to work |

### Tmux Integration

Gas Town agents often run in tmux sessions. ogent provides commands to interact with them:

| Command | Description |
|---------|-------------|
| `M-x ogent-gastown-tmux-sessions` | List all tmux sessions |
| `M-x ogent-gastown-tmux-attach` | Attach to a session (term/vterm/eat) |
| `M-x ogent-gastown-tmux-preview` | Preview session output in popup |
| `M-x ogent-gastown-tmux-send` | Send command to a session |

### Header-Line Status

When `ogent-gastown-mode` is active, the header-line shows:
- **Hook status**: Currently assigned work (🪝 icon)
- **Mail count**: Unread messages (📬 icon)
- **Convoy progress**: Active batch work status

### Issue Tracking (Beads)

Gas Town uses Beads (`bd`) for issue tracking. ogent integrates with this system:

```elisp
;; View issue details
(ogent-gastown-show-issue "og-123")

;; Claim an issue
(ogent-gastown-claim-issue "og-123")

;; Close an issue
(ogent-gastown-close-issue "og-123" "Completed implementation")
```

For the full issue browser with filtering and AI-assisted editing, see `M-x ogent-issues`.
