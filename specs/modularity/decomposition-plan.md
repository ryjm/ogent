...
- **Autoload preservation (elisp landmine, corrected per fresh-eyes audit).** A generated autoload
  ...
  - **Manual redirect form** already present (e.g. ui-armory `;;;###autoload (autoload
    'ogent-armory-home-dispatch "ogent-ui-armory" nil t)` @1227; ui `(autoload 'ogent-request
  ...
  → redirect to `"ogent-zen"`); ui-armory's 17 view commands keep their current targets (the manual
  `"ogent-ui-armory"` redirects stay in the façade, cookies off the moved defuns); ui's **14 cookied
...
## 1. ogent-ui-armory.el → WINNER: Candidate A (core + 9 view siblings + façade; 10 buffer surfaces)
...
core→view→façade. Matches the project's flat-sibling idiom 1:1 (like the `ogent-armory-*` family).

| New file (lisp/ui/) | Clusters | ~LOC | provide | requires | autoloads |
|---|---|---|---|---|---|
| `ogent-ui-armory-core.el` | prelude+custom+const+state+helpers (3 macros, magit gate, section nav, insert helpers) | ~870 | `ogent-ui-armory-core` | external only (cl-lib, org, magit-section eval-and-compile, ogent-armory*, tabulated-list, transient) | none |
| `ogent-ui-armory-home.el` | Home cockpit + transient | ~414 | …`-home` | core | home, home-dispatch |
| `ogent-ui-armory-agents.el` | AgentList | ~131 | …`-agents` | core | agents |
| `ogent-ui-armory-org-chart.el` | OrgChart | ~73 | …`-org-chart` | core | org-chart |
| `ogent-ui-armory-agent.el` | AgentProfile + AgentMgmt | ~427 | …`-agent` | core | agent, create-agent, clone-agent, archive-agent |
| `ogent-ui-armory-jobs.el` | Jobs | ~261 | …`-jobs` | core | jobs, create-job |
| `ogent-ui-armory-tasks.el` | Tasks | ~493 | …`-tasks` | core | tasks, create-task |
| `ogent-ui-armory-conversations.el` | ConvList + ConvDetail (kept together, bidirectional) | ~714 | …`-conversations` | core | conversations, conversation |
| `ogent-ui-armory-search.el` | Search | ~84 | …`-search` | core | search |
| `ogent-ui-armory-apps.el` | Apps | ~111 | …`-apps` | core | apps, open-app |
| `ogent-ui-armory.el` (façade) | Bootstrap (setup-section-keymaps + evil/magit quartet) + provide | ~90 | `ogent-ui-armory` | core + all 9 views | none |

Autoload tally moved: 2+1+1+4+2+2+2+1+2 = **17** (unchanged set).

**Façade structure:** `(require 'ogent-ui-armory-core)` → 9 view requires (any order; all need only
core) → move `ogent-armory-ui--section-keymaps`/`--setup-section-keymaps` + evil quartet
...
1. Extract `-core` (must be first: every view requires it; the `define-section-mode`/`with-section`
   ...
   `(require 'ogent-ui-armory-core)`. Gate.
...
**Top risks (from findings §6):** (a) every view file must `(require 'ogent-ui-armory-core)` at top
...
**line 1227** already targets the façade `"ogent-ui-armory"`: **keep it verbatim in the façade**
(move `ogent-armory-home-dispatch`'s defun to the home sibling WITHOUT a cookie; do NOT retarget to
`"ogent-ui-armory-home"`, which would skip the façade bootstrap on cold invoke);
`ogent-ui-armory-conversations` must not collide with external `ogent-armory-conversations`;
(d) views never `require` each other (keep autoload+`fboundp` guard on `jobs--goto`@1183);
(e) keep `defgroup ogent-ui-armory` + 11 customs + 4 faces in core.
...
|---|---|---|---|---|---|
| `ogent-zen-core.el` | base tower: predicates, status-parse, formatters, generic render utils, title helpers, scope primitives; `defgroup`+21 customs+4 faces; `cl-defstruct ogent-zen-scope` | ~600 | `ogent-zen-core` | cl-lib, org, org-element, subr-x | none |
...
| `ogent-zen-workspace.el` | workspace/context inference | ~286 | `ogent-zen-workspace` | `ogent-zen-core`, `ogent-context` | none |
...
|---|---|---|---|---|---|
| `ogent-ui.el` (façade) | none (hosts both load-time forms) | ~135 | `ogent-ui` | core, format, toolcalls, engine, send, preview, dispatch | redirect forms for all moved cmds |
| `ogent-ui-core.el` | CORE: request struct, central state, cross-cut customs/consts, render-prompt, scope helpers | ~150 | `ogent-ui-core` | cl-lib, ogent-core; **+ `declare-function` stubs** for `ogent-ui-insert-response-block` (format) & `ogent-ui-prepare-response-block` (engine): function-quoted by `ogent-response-function`/`--set-response-function` but cannot be `require`d (cycle) | none |
| `ogent-ui-format.el` | E org-formatting + D window/scroll | ~180 | `ogent-ui-format` | core, org (zen via declare) | none |
| `ogent-ui-toolcalls.el` | F tool-exec + G tool-drawer + H inline-diff (kept whole) | ~930 | `ogent-ui-toolcalls` | core, format, ogent-tool-approval, ogent-ledger, ogent-edit-format | none |
...
| `ogent-issues-evil.el` | C21 evil setup (self-registers via `with-eval-after-load 'evil`) | ~55 | `ogent-issues-evil` | `declare-function` mode-maps/hooks from core (`ogent-issues-mode-map`, `-detail-mode-map`, + hooks); no hard require (callback fires after evil loads) | none |
...
## 5. ogent-armory.el → LEAVE-ALONE (B11 justified monolith)

1743 LOC / 109 defs / 25 requirers. **Do not split.** Grounded positively: 100% of defs are Armory
...
Candidate A′: extract C1+C2+C3 (~318 LOC) into `ogent-armory-store.el` behind the façade (façade
...
|------|-----|---------|----------------------|---------------|------|
| ogent-ui-armory.el | 3670 | should-split | 11 (core+10 views+façade) | ~870 (core) | low (acyclic, no shared state) |
...
| ogent-armory.el | 1743 | **leave-alone (B11)** | 1 (unchanged) | 1743 | n/a |

Total new sibling files (winners): 10 (ui-armory) + 4 (zen) + 7 (ui) + 3 (issues) = **24 new files**,
...
Order of execution (independent; recommended by ROI/safety): **ui-armory (cleanest, highest LOC win)
→ issues-B (lowest risk) → zen-A → ui-A**, then optional evolutions (zen-B review/render, ui-B,
issues-A) as separate follow-ups. armory untouched.
