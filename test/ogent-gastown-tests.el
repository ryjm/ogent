...
(ert-deftest ogent-gastown-test-hook-show-uses-hook-command-root ()
  ...
  (let ((captured nil)
    ...
    (cl-letf (((symbol-function 'ogent-gastown--hook-command-root)
      ...
      (should (equal captured
                     (list hook-args "*gt hook*" hook-dir))))))

(ert-deftest ogent-gastown-test-hook-attach-uses-hook-command-root ()
  "Hook attach should run in hook-target-aware command directory."
  (let ((captured nil)
        (hook-dir "/tmp/ogent-test-hook/"))
    (cl-letf (((symbol-function 'read-string)
               (lambda (&rest _) "og-123"))
              ((symbol-function 'ogent-gastown--hook-command-root)
               (lambda () hook-dir))
      ...
      (should (equal captured
                     (list '("hook" "og-123") t hook-dir))))))

(ert-deftest ogent-gastown-test-fetch-all-hook-uses-hook-command-root ()
  ...
  (let ((hook-run-dir nil)
    ...
    (with-temp-buffer
      (cl-letf (((symbol-function 'ogent-gastown--hook-command-root)
                 (lambda () hook-root))
                ((symbol-function 'ogent-gastown--hook-status-command-args)
                 (lambda () hook-args))
                ((symbol-function 'ogent-gastown--run-async-cached)
                 (lambda (args callback &optional _error-callback _raw-output run-directory)
                   (when (equal args hook-args)
        ...
        (should (equal hook-run-dir hook-root))))))
