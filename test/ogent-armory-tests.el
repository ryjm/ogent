...
(ert-deftest ogent-armory-session-detail-keeps-src-block-headings-in-trace ()
    ...
    (let* ((session-dir (ogent-armory-sessions-directory dir "cto"))
      ...
      (ogent-armory--write-file
       ...
       (concat
        ...
        "last line\n"
        "#+end_src\n"))
      (let ((trace (plist-get (ogent-armory-session-detail file "cto")
                              :runtime-trace)))
...
(ert-deftest ogent-armory-todo-sequence-orders-active-and-done ()
  "The TODO sequence puts RUNNING active and FAILED after the bar."
  (let ((seq (ogent-armory--todo-sequence)))
...
(ert-deftest ogent-armory-scaffold-index-declares-todo-keywords ()
  "A scaffolded armory index carries a #+TODO: line with its states."
  (ogent-armory-test-with-temp-dir dir
    (ogent-armory-scaffold dir "Acme" :kind "root" :create-editor nil)
    (let ((content (ogent-armory-test--slurp
                    (expand-file-name "index.org" dir))))
      (should (string-match-p "^#\\+TODO:.*RUNNING" content))
      (should (string-match-p "^#\\+TODO:.*FAILED" content)))))

(ert-deftest ogent-armory-register-todo-keywords-is-idempotent ()
