...
(ert-deftest ogent-gts-test-ready-bead-ids-uses-bd-ready ()
  "Ready bead candidates are populated via `bd ready --json`."
  (let ((observed-default-directory nil))
               ...
               (lambda (callback &optional _error-callback)
                 ...
                 (funcall callback
                          (list '(:id "og-b")
                                ...
                                '(:id "og-b")))
                 nil)))
      (should (equal (ogent-gastown--ready-bead-ids) '("og-a" "og-b")))
      (should (equal observed-default-directory temporary-file-directory)))))

(ert-deftest ogent-gts-test-get-sling-targets-includes-rigs-and-recipients ()
  "Sling targets include rig names plus recipient-style agent addresses."
  (let ((ogent-gastown--rigs-data
         (list '(:name "beads")
               '(:name "ogent")
               '(:name nil))))
    (cl-letf (((symbol-function 'ogent-gastown--get-mail-recipients)
               (lambda () '("ogent/crew/ritchie" "mayor/"))))
      (let ((targets (ogent-gastown--get-sling-targets)))
        (should (member "beads" targets))
        (should (member "ogent" targets))
        (should (member "ogent/crew/ritchie" targets))
        (should (member "mayor/" targets))))))

(ert-deftest ogent-gts-test-sling-uses-ready-completion-without-context ()
  "Sling prompts from ready beads and target list when point has no context."
  (let ((captured-bead-candidates nil)
        (captured-target-candidates nil)
        (run-async-args nil)
        (run-async-directory nil))
    (cl-letf (((symbol-function 'ogent-gastown--bead-id-at-point)
               (lambda () nil))
              ((symbol-function 'ogent-gastown--sling-target-at-point)
               (lambda () nil))
              ((symbol-function 'ogent-gastown--ready-bead-ids)
               (lambda () '("og-1" "og-2")))
              ((symbol-function 'ogent-gastown--get-sling-targets)
               (lambda () '("ogent" "ogent/crew/ritchie")))
              ((symbol-function 'ogent-gastown--identity-command-root)
               (lambda () temporary-file-directory))
              ((symbol-function 'completing-read)
               (lambda (prompt collection &optional _predicate _require-match
                               _initial-input _hist _def _inherit-input-method)
                 (cond
                  ((string-match-p "Bead ID to sling" prompt)
                   (setq captured-bead-candidates collection)
                   "og-2")
                  ((string-match-p "Sling to" prompt)
                   (setq captured-target-candidates collection)
                   "ogent/crew/ritchie")
                  (t ""))))
              ((symbol-function 'ogent-gastown-status--run-async)
               (lambda (args callback &optional _error-callback _raw run-directory)
                 (setq run-async-args args)
                 (setq run-async-directory run-directory)
                 (funcall callback nil)))
              ((symbol-function 'ogent-gastown-cache-invalidate) #'ignore)
              ((symbol-function 'ogent-gastown-refresh)
               (lambda (&rest _) nil))
              ((symbol-function 'message) #'ignore))
      (ogent-gastown-sling)
      (should (equal captured-bead-candidates '("og-1" "og-2")))
      (should (equal captured-target-candidates '("ogent" "ogent/crew/ritchie")))
      (should (equal run-async-args '("sling" "og-2" "ogent/crew/ritchie")))
      (should (equal run-async-directory temporary-file-directory)))))

(ert-deftest ogent-gts-test-sling-context-bead-skips-bead-prompt ()
  "Sling uses bead at point and prompts only for target."
  (let ((bead-prompted nil)
        (ready-loaded nil)
        (run-async-args nil))
    (cl-letf (((symbol-function 'ogent-gastown--bead-id-at-point)
               (lambda () "og-context"))
              ...
              ((symbol-function 'ogent-gastown--ready-bead-ids)
               (lambda ()
                 ...
                 '("og-1")))
              ((symbol-function 'ogent-gastown--get-sling-targets)
               (lambda () '("ogent/crew/ritchie")))
              ((symbol-function 'completing-read)
               (lambda (prompt _collection &optional _predicate _require-match
                               _initial-input _hist _def _inherit-input-method)
                 (if (string-match-p "Bead ID to sling" prompt)
                     (progn
                       (setq bead-prompted t)
                       "unexpected")
                   "ogent/crew/ritchie")))
              ((symbol-function 'ogent-gastown-status--run-async)
               (lambda (args callback &optional _error-callback _raw _run-directory)
              ...
              ((symbol-function 'message) #'ignore))
      (ogent-gastown-sling)
      (should-not bead-prompted)
      (should-not ready-loaded)
      (should (equal run-async-args '("sling" "og-context" "ogent/crew/ritchie"))))))

(ert-deftest ogent-gts-test-sling-context-target-skips-target-prompt ()
  "Sling uses target at point and prompts only for bead ID."
  (let ((target-prompted nil)
        (run-async-args nil))
    (cl-letf (((symbol-function 'ogent-gastown--bead-id-at-point)
               (lambda () nil))
              ((symbol-function 'ogent-gastown--sling-target-at-point)
              ...
              ((symbol-function 'completing-read)
               (lambda (prompt _collection &optional _predicate _require-match
                               _initial-input _hist _def _inherit-input-method)
                 (if (string-match-p "Sling to" prompt)
                     (progn
                       (setq target-prompted t)
                       "unexpected")
                   "og-ctx")))
              ((symbol-function 'ogent-gastown-status--run-async)
               (lambda (args callback &optional _error-callback _raw _run-directory)
              ...
              ((symbol-function 'message) #'ignore))
      (ogent-gastown-sling)
      (should-not target-prompted)
      (should (equal run-async-args '("sling" "og-ctx" "ogent/crew/ritchie"))))))
