;;; ogent-cabinet-conversations-tests.el --- Tests for Cabinet conversations -*- lexical-binding: t; -*-

;;; Commentary:
;; Tests for the Org-backed canonical Cabinet conversation store.
...
(defmacro ogent-cabinet-conversations-test-with-temp-dir (var &rest body)
  "Bind VAR to a temporary directory while running BODY."
  (declare (indent 1) (debug t))
  `(let ((,var (make-temp-file "ogent-cabinet-conversations-" t)))
     (unwind-protect
         (progn ,@body)
       (when (file-directory-p ,var)
         (delete-directory ,var t)))))

(ert-deftest ogent-cabinet-conversation-round-trips-index ()
...
(ert-deftest ogent-cabinet-conversation-appends-turns-and-events ()
  ...
  (ogent-cabinet-conversations-test-with-temp-dir dir
    ...
    (let ((user-turn (ogent-cabinet-conversation-append-turn
          ...
          (agent-turn (ogent-cabinet-conversation-append-turn
    ...
    (ogent-cabinet-conversation-append-event
     ...
     :ts "2026-05-06T10:01:00Z"
     :payload "role=user")
    (let ((turns (ogent-cabinet-conversation-read-turns dir "conv-2"))
          (events (ogent-cabinet-conversation-read-events dir "conv-2")))
...
(ert-deftest ogent-cabinet-conversation-list-sorts-by-activity ()
...
(ert-deftest ogent-cabinet-conversation-migrates-legacy-session ()
  ...
  (ogent-cabinet-conversations-test-with-temp-dir dir
    ...
    (let* ((session-file (expand-file-name
      ...
      (let* ((conversation-file
              (ogent-cabinet-conversation-migrate-session dir session-file "cto"))
             (conversation (ogent-cabinet-conversation-read
...
(provide 'ogent-cabinet-conversations-tests)

;;; ogent-cabinet-conversations-tests.el ends here
