...
(ert-deftest ogent-armory-status-evil-overrides-dispatch-keymap ()
  ...
  (let ((ogent-armory-status-mode-hook nil)
    ...
    (should (memq #'ogent-armory-status--evil-local-keys
                  ogent-armory-status-mode-hook))
    (should (memq #'evil-normalize-keymaps ogent-armory-status-mode-hook))))

(ert-deftest ogent-armory-status-evil-local-keys-match-magit-navigation ()
  "Armory status adds Evil normal-state Magit navigation keys."
  (let (keys)
    (with-temp-buffer
    ...
    (dolist (binding '(("j" evil-next-line)
                       ...
                       ("G" evil-goto-line)
                       ("gr" ogent-armory-status-refresh)
                       ("gj" ogent-armory-status-next-section)
                       ("gk" ogent-armory-status-previous-section)
                       ("ZZ" quit-window)
                       ("ZQ" quit-window)))
