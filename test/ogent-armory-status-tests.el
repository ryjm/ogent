...
(ert-deftest ogent-armory-status-action-keys-are-discoverable ()
  "The status map exposes the bare-key action contract and shared jumps."
  (dolist (pair `(("?" . ,#'ogent-armory-status-dispatch)
                  ("g" . ,#'ogent-armory-status-refresh)
                  ("n" . ,#'ogent-armory-status-next-item)
                  ("p" . ,#'ogent-armory-status-previous-item)
  ...
  (dolist (pair `(("h" . ,#'ogent-armory-home)
                  ("g" . ,#'ogent-armory-status)
                  ("a" . ,#'ogent-armory-agents)
                  ("o" . ,#'ogent-armory-org-chart)
                  ...
                  ("A" . ,#'ogent-armory-apps)
                  ("d" . ,#'ogent-armory-data)
                  ("u" . ,#'ogent-armory-schedule)
                  ("v" . ,#'ogent-armory-git-status)))
    (should (eq (lookup-key ogent-armory-jump-map (kbd (car pair)))
                (cdr pair))))
  (dolist (old-key '("C-c m" "C-c ?" "C-c g" "C-c n" "C-c p" "C-c r"
                     "C-c e" "C-c E" "C-c P" "C-c J" "C-c N"
                     "C-c h" "C-c a" "C-c t" "C-c c" "C-c s" "C-c A"))
    (should-not (commandp (lookup-key ogent-armory-status-mode-map
                                      (kbd old-key)))))
  (ogent-armory-status-test-with-temp-dir dir
    (ogent-armory-scaffold dir "Company" :kind "root" :create-editor nil)
      ...
      (unwind-protect
          (with-current-buffer buffer
            (let ((header (substring-no-properties
                           (ogent-armory-status--header-line))))
              (dolist (key '("?:menu" "j:jump" "g:refresh"))
                (should (string-match-p (regexp-quote key) header)))
              (should-not (string-match-p "C-c" header))
              (should-not (string-match-p "RET visit" header))
              (should-not (string-match-p "TAB fold" header))))
        (when (buffer-live-p buffer)
          (kill-buffer buffer))))))
...
(ert-deftest ogent-armory-status-section-keybindings-are-discoverable ()
  ...
  (dolist (pair `(("TAB" . ,#'ogent-section-toggle)
  ...
  (save-window-excursion
    ...
    (with-current-buffer "*ogent-armory-status-help*"
      (let ((text (buffer-substring-no-properties (point-min) (point-max))))
        ...
        (should (string-match-p (regexp-quote "Press ? for the action menu") text))
        (should (string-match-p "Agent" text))
        (should (string-match-p "P opens profile" text))
        (should (string-match-p "C creates a job" text))
        (should (string-match-p "R runs with a prompt" text))
        (should (string-match-p "Job" text))
        (should (string-match-p "e edits metadata" text))
        (should (string-match-p "E edits the prompt/body" text))
        (should (string-match-p "J opens the job list" text))
        (should (string-match-p "j h Home" text))
        (should (string-match-p "j g Graph" text))
        (should (string-match-p "j t Tasks" text))
        (should (string-match-p "i opens Issues" text))
        (should (string-match-p (regexp-quote "? opens the Transient menu") text))
        (should (string-match-p "C-u g forces a rebuild" text))
        (should (string-match-p ", opens Settings" text))
        (should (string-match-p "/ opens the command palette" text))
        (should (string-match-p "TAB toggles a section" text))
        (should (string-match-p "\\^ moves to the parent section" text))))))
...
(ert-deftest ogent-armory-status-evil-local-keys-cover-status-map ()
  "Evil setup mirrors advertised status commands into normal and motion states."
  (let ((ogent-enable-evil-bindings t)
        (ogent-armory-status-mode-hook nil)
        states maps defined)
    (cl-letf (((symbol-function 'evil-set-initial-state)
               (lambda (mode state)
                 (push (cons mode state) states)))
              ...
              ((symbol-function 'evil-define-key*)
               (lambda (evil-states keymap key command)
                 (push (list evil-states keymap (key-description key) command)
                       defined)))
              ((symbol-function 'evil-normalize-keymaps)
               (lambda (&rest _) nil)))
      (ogent-armory-status--setup-evil))
    ...
    (should (member (cons ogent-armory-status-mode-map 'motion) maps))
    (should (memq #'evil-normalize-keymaps ogent-armory-status-mode-hook))
    (dolist (binding '(("?" ogent-armory-status-dispatch)
                       ("g" ogent-armory-status-refresh)
                       ("n" ogent-armory-status-next-item)
                       ("p" ogent-armory-status-previous-item)
                       ("i" ogent-armory-status-open-issues)
                       ...
                       ("M-p" ogent-section-prev)
                       ("^" ogent-section-up)
                       ("j h" ogent-armory-home)
                       ("j g" ogent-armory-status)
                       ("j a" ogent-armory-agents)
                       ("j o" ogent-armory-org-chart)
                       ("j t" ogent-armory-tasks)
                       ("j c" ogent-armory-conversations)
                       ("j j" ogent-armory-jobs)
                       ("j s" ogent-armory-search)
                       ("j A" ogent-armory-apps)
                       ("j d" ogent-armory-data)
                       ("j u" ogent-armory-schedule)
                       ("j v" ogent-armory-git-status)))
      (should (member (list '(normal motion)
