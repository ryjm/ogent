...
(defun ogent-cabinet-runner-test--wait (process)
  ...
  (let* ((plan (process-get process 'ogent-cabinet-plan))
         ...
         (conversation-id (process-get process 'ogent-cabinet-conversation-id))
         (deadline (+ (float-time) 5)))
    (while (and (< (float-time) deadline)
                (or (memq process ogent-cabinet-runner--processes)
                    (let ((status (ignore-errors
                                    (plist-get
                                     (ogent-cabinet-conversation-read
...
(ert-deftest ogent-cabinet-runner-prompts-for-org-output ()
  ...
  (ogent-cabinet-runner-test-with-temp-dir dir
    ...
    (let* ((plan (ogent-cabinet-runner-plan
      ...
      (should (string-match-p "Example Cabinet response" prompt))
      (should (string-match-p "Machine footer" prompt))
      (should (string-match-p "Before sending" prompt))
      (should (string-match-p "Cabinet run context" prompt))
      (should (string-match-p "Runtime: native" prompt)))))

(ert-deftest ogent-cabinet-runner-selects-implementation-template ()
...
(ert-deftest ogent-cabinet-runner-selects-review-template-from-job ()
...
(ert-deftest ogent-cabinet-runner-adds-action-contract-for-leads ()
  ...
  (ogent-cabinet-runner-test-with-temp-dir dir
    ...
    (let* ((plan (ogent-cabinet-runner-plan
      ...
      (should (string-match-p "#\\+begin_cabinet-actions" prompt))
