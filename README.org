...
- *Commands, not conversations*: =C-c . q= asks about the current subtree in place. =C-c . ?= shows the ask menu. =C-c . p= opens full dispatch. =C-c . E= requests code edits.
...
- Press =C-c . q= to ask about the subtree at point; ogent inserts a folded =Request= and streams the =Response= below that subtree.
- Press =C-c . ?= when you are unsure: the ask menu names the active scope and offers inline ask, popup ask, context preview, and the full dispatcher.
