...
Cabinet turns a directory of Org files into a local AI-team workspace. Open
=M-x ogent-cabinet-home= or press =C-c . j=. The Cabinet surface includes
...
artifacts, and a schedule calendar. In =M-x ogent-cabinet-tasks=, press =c= or
=C-c c= to capture a manual Inbox task with a short TODO-style prompt path.

| Key       | Command                         | Description                 |
|-----------+---------------------------------+-----------------------------|
| =C-c . j= | =ogent-cabinet-home=            | Cabinet Home                |
| =C-c . K= | =ogent-cabinet-status=          | Cabinet graph/status        |
| =C-c . y= | =ogent-cabinet-agents=          | Cabinet agents              |
| =C-c . Y= | =ogent-cabinet-agent=           | Cabinet agent profile       |
| =C-c . B= | =ogent-cabinet-org-chart=       | Cabinet org chart           |
| =C-c . ;= | =ogent-cabinet-data=            | Cabinet data browser        |
| =C-c . I= | =ogent-cabinet-tasks=           | Cabinet task board          |
| =C-c . O= | =ogent-cabinet-conversations=   | Cabinet conversations       |
| =C-c . N= | =ogent-cabinet-actions=         | Cabinet action approvals    |
| =C-c . J= | =ogent-cabinet-schedule=        | Day/week/month schedule     |
| =C-c . Q= | =ogent-cabinet-agenda=          | Org agenda over Cabinet     |
| =C-c . := | =ogent-cabinet-git-status=      | Cabinet git status          |
| =C-c . /= | =ogent-cabinet-command-palette= | Ranked command palette      |
| =C-c . ,= | =ogent-cabinet-settings=        | Cabinet settings            |
| =C-c . .= | =ogent-cabinet-help=            | Cabinet help                |
| =C-c . '= | =ogent-cabinet-onboard=         | Onboard Cabinet             |
| ~C-c . =~ | =ogent-cabinet-registry-import= | Import Cabinet template     |
| =C-c . _= | =ogent-cabinet-backup=          | Back up Cabinet             |
| =C-c . V= | =ogent-cabinet-search=          | Cabinet-wide search         |
| =C-c . W= | =ogent-cabinet-apps=            | Generated app artifacts     |
| =C-c . X= | =ogent-cabinet-create-agent=    | Create Cabinet agent        |
| =C-c . Z= | =ogent-cabinet-create-job=      | Create Cabinet job          |
