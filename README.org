...
- Use =C-c . ?= to ask a question about the current Org subtree or buffer; the minibuffer prompt names the scope.
...
| =C-c . ?= | =ogent-ask=                   | Ask about current subtree or buffer      |
